...
class Attack:
    ...
    def layers_(self, model):
        ...
        if self.get_layers is not None:
            ...
            self.layers = [i for i in layers if named_parameters[named_parameters_keys[i]].requires_grad is True]
        else:
            named_parameters = dict(model.named_parameters())
            named_parameters_keys = list(named_parameters.keys())
            self.layers = [i for i in range(len(named_parameters_keys)) if named_parameters[named_parameters_keys[i]].requires_grad is True]
