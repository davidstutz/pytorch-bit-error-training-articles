...
class RandomAttack(Attack):
    ...
    def run(self, model, testset, objective):
        ...
        forward_model, self.quantization_contexts = self.quantize(model)
        ...
        for i in range(self.epochs):
            ...
            if self.epochs <= 1:
                ...
                norm = self.norm(model, forward_model, self.layers, self.quantization, self.quantization_contexts)
                if self.progress is not None:
                    self.progress('attack weights', self.epochs * len(testset) - 1, self.epochs * len(testset), info='norm=%g' % norm, width=10)
                #for parameter in success_model.parameters():
            else:
                total_error = 0
                norm = self.norm(model, forward_model, self.layers, self.quantization, self.quantization_contexts)
