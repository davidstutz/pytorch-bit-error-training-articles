...
class Main:
    def __init__(self, args=None):
        ...
        train_transform = torchvision.transforms.Compose([
            ...
            #common.autoaugment.Debug(),
            common.torch.CutoutAfterToTensor(n_holes=1, length=cutout, fill_color=data_mean),
            #common.autoaugment.Debug(),
        ])
    ...
    def get_parser(self):
        ...
        parser = argparse.ArgumentParser(description='Train quantized model with weight clipping.')
        parser.add_argument('--architecture', type=str, default='resnet50')
        parser.add_argument('--directory', type=str, default='./checkpoints/')
        parser.add_argument('--normalization', type=str, default='rebn')
    ...
    def main(self):
        ...
        def average_simple_curriculum(attack, loss, perturbed_loss, epoch, threshold=2):
            if loss < threshold:
        ...
        projection = attacks.weights.projections.BoxProjection(-self.args.w_max, self.args.w_max)
        ...
        attack.initialization = attacks.weights.initializations.BitRandomInitialization(self.args.p)
        attack.quantization = quantization
        attack.projection = projection

        trainer = common.train.AverageWeightsTraining(
            self.model, self.trainloader, self.testloader, self.optimizer, self.scheduler, attack, objective,
            writer=writer, cuda=self.args.cuda)
        trainer.quantization = quantization
        trainer.projection = projection
        trainer.curriculum = average_simple_curriculum

        if start_epoch < self.epochs:
            for epoch in range(start_epoch, self.epochs):
                _, quantized_model, _ = trainer.step(epoch)
                writer.flush()
                self.checkpoint(model_file, quantized_model, epoch)
            self.checkpoint(model_file, quantized_model)
        else:
            # Note that the unquantized model does generally not have good test error for low precisions.
            quantized_model, _ = trainer.quantize()

        quantized_model.eval()
        error = common.test.test(quantized_model, self.testloader, cuda=self.args.cuda)
        log('error: %g' % error)
